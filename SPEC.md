...
- 現行バージョン: v1.22
- アプリ表示名: 「Bio-Scheduler」（タイトル先頭にバイオリズム波のアイコンを表示）
...
## 6.5 週/2週グリッドの表示補助（v1.22）

- **隠れた時間指定予定のバッジ**: スクロールで画面外（上/下）に出た時間指定予定を、
  **曜日列ごと**に上端「↑N」／下端「↓N」（オレンジ）で表示。タップでその予定までスクロール。
  各予定 div に `data-di`（列インデックス）を付与し、`renderWeek` 内 `updateSchedMore()` が
  スクロール・終日欄リサイズに追従して列ごとに件数を再計算する。終日予定の「+N」とは別系統。
- **週開始曜日の切替**: `setWeekStart()` は今表示中の期間と**最も重なる週**を選ぶため、
  日↔月を往復しても前後の週へドリフトしない（例: 14日(日)始まり ⇄ 15日(月)始まり）。
